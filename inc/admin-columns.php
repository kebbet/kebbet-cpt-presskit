...
namespace cpt\kebbet\presskit\admincolumns;
...
function set_admin_column_list( $columns ) {
	$columns['modified']  = __( 'Last modified', 'kebbet-cpt-presskit' );
	$columns['thumbnail'] = __( 'Featured image', 'kebbet-cpt-presskit' );
	return $columns;
}
...
function populate_custom_columns( $column, $post_id ) {
	...
	if ( 'thumbnail' === $column ) {
		...
		if ( $thumbnail ) {
			...
			echo __( 'No image is set.', 'kebbet-cpt-presskit' );
		}
	}
}
